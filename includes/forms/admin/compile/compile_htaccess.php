...
if ($compile_htaccess == true) {
	...
	$updated = updateHashInJson($htaccess_template, $jsonFilePath);
	$message = 'Unchanged.';

	$destination = $absolute_root . '.htaccess';

// If the template has been revised or the .htaccess file doesn't exist,
// then proceed:
...
// Retain it if we're not on localhost:
		...
		file_put_contents($destination, $htaccess);
	}
