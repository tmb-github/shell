// 4.9.2 (2018-12-17)
// With various updates by me, the latest on 2022-03-09:
(function () {
(function () {
    'use strict';
    ...
    var deprecated = {
      ...
      keyIdentifier: 1,
// 2022-03-09:
// Event.path is deprecated and will be removed in January 2023
// Use composedPath() instead.
			path: 1
    };
...
// OLD:            numeric = parseInt(numeric.substr(1), 16);
            numeric = parseInt(numeric.substring(1), 16);
          } else {
...
// 2021-11-28
// substr() is deprecated!
// OLD:            if (languages.indexOf(',' + language.substr(0, 2) + ',') !== -1) {
            if (languages.indexOf(',' + language.substring(0, 2) + ',') !== -1) {
// OLD:              language = language.substr(0, 2);
...
// 2021-11-28
// substr() is deprecated!
// OLD:      var before = trimCount(caretContainer.data.substr(0, pos.offset()));
      var before = trimCount(caretContainer.data.substring(0, pos.offset()));
// OLD:      var after = trimCount(caretContainer.data.substr(pos.offset()));
      var after = trimCount(caretContainer.data.substring(pos.offset()));
...
// 2021-11-28
// substr() is deprecated!
// OLD:            content = content.substring(0, index) + replace + content.substr(index + search.length);
...
// 2021-11-28
// substr() is deprecated!
// OLD:            text(decode(html.substr(index, matches.index - index)));
// substr(x, y) -> substring(x, x + y)
            text(decode(html.substring(index, matches.index)));
          }
          if (value = matches[6]) {
            value = value.toLowerCase();
            if (value.charAt(0) === ':') {
...
// 2021-11-28
// substr() is deprecated!
// OLD:              text(decode(html.substr(matches.index)));
              text(decode(html.substring(matches.index)));
...
// 2021-11-28
// substr() is deprecated!
// OLD:                  textData = html.substr(index, matches.index - index);
// substr(x, y) -> substring(x, x + y)
                  textData = html.substring(index, matches.index);
                }
                index = matches.index + matches[0].length;
              } else {
// 2021-11-28
// substr() is deprecated!
// OLD:                textData = html.substr(index);
                textData = html.substring(index);
                index = html.length;
              }
              if (isValidElement) {
...
// 2021-11-28
// substr() is deprecated!
// OLD:            if (!settings.allow_conditional_comments && value.substr(0, 3).toLowerCase() === '[if') {
            if (!settings.allow_conditional_comments && value.substring(0, 3).toLowerCase() === '[if') {
              value = ' ' + value;
            }
            comment(value);
...
// 2021-11-28
// substr() is deprecated!
// OLD:          text(decode(html.substr(index)));
          text(decode(html.substring(index)));
        }
...
// 2021-11-28
// substr() is deprecated!
// OLD:            node.value = unescape(node.value).substr(14);
            node.value = unescape(node.value).substring(14);
          }
        }
      });
...
// 2021-11-28
// substr() is deprecated!
// OLD:            internalContent = internalContent.substr(mceInternalUrlPrefix.length).split(',');
            internalContent = internalContent.substring(mceInternalUrlPrefix.length).split(',');
            return {
...
// 2021-11-28
// substr() is deprecated!
// OLD:            plugin = plugin.substr(1, plugin.length);
// substr(x, y) -> substring(x, x + y)
            plugin = plugin.substring(1, plugin.length + 1);
...
// 2021-11-28
// substr() is deprecated!
// OLD:        if (tu === uu || tu.charAt(tu.length - 1) === '/' && tu.substr(0, tu.length - 1) === uu) {
        if (tu === uu || tu.charAt(tu.length - 1) === '/' && tu.substring(0, tu.length - 1) === uu) {
          return tu;
        }
