/**
* serverErrorTest.mjs
* Copyright (c) 2019-2023 Thomas M. Brodhead <https://bmt-systems.com>
* Released under the MIT license
* Date: 2023-11-01
*/

// Every mjs file has a main() function that is called by common.mjs inner().
//
// Functions from common.mjs are accessed as methods of the 'o' object:
//
// o.commonMjsFunction();
//
// functions local to this module that need access to the 'o' object and its
// methods should receive the 'o' object as a parameter when called:
//
// localFunction(o);
//
// Local functions may also be called with o.[mjsFilePrefix].localFunction();
// but then, in their body, they must have an 'o' variable set equal to
// their 'this' keyword, as is done in main().

var formLogic;
var main;
var returnMetaData;


main = function () {

	var metaData;
	var o;

// 'this' is the outer 'o' via .bind(o), so the outer 'o' === inner 'o':
	o = this;

	metaData = returnMetaData(o);
	o.reviseMetaData(metaData);

	o.serverErrorTest.formLogic();

};

formLogic = function () {

	var formOnSubmit;
	var o;

// 'this' is the outer 'o' via .bind(o), so the outer 'o' === inner 'o':
	o = this;

	formOnSubmit = function (event) {
		var ajaxURL;
		var contactFormMutate;
		var hideRandomWordRecognized;
		var hideRandomWordUnrecognized;
		var randomWord;
		var randomWordInput;
		var showRandomWordRecognized;
		var showRandomWordUnrecognized;
		var submit;

		event.preventDefault();

		contactFormMutate = function () {

			var ajaxResponse;

			if (randomWord === '') {
				alert("Random word required.\n\nPlease type random word and re-submit.\n");
			} else {
				submit.disabled = true;
// response:
				ajaxResponse = function (userData) {
					randomWordInput.value = '';
					submit.disabled = false;
					if (userData === 'true') {
						showRandomWordRecognized();
					} else {
						showRandomWordUnrecognized();
					}
				};
			}
			o.ajax.post(ajaxURL, {'random_word': randomWord}, ajaxResponse, true);

		};

		hideRandomWordRecognized = function () {
			(function (message) {
				if (message) {
					message.classList.remove('show');
				}
			}(document.querySelector('.random-word-recognized')));
		};

		hideRandomWordUnrecognized = function () {
			(function (message) {
				if (message) {
					message.classList.remove('show');
				}
			}(document.querySelector('.random-word-unrecognized')));
		};

		showRandomWordRecognized = function () {
			(function (message) {
				if (message) {
					message.classList.add('show');
				}
			}(document.querySelector('.random-word-recognized')));
		};

		showRandomWordUnrecognized = function () {
			(function (message) {
				if (message) {
					message.classList.add('show');
				}
			}(document.querySelector('.random-word-unrecognized')));
		};

		randomWordInput = document.getElementById('random-word-input');
		randomWord = randomWordInput.value.trim();
		randomWordInput.addEventListener('focus', function () {
			hideRandomWordRecognized();
			hideRandomWordUnrecognized();
		});

		submit = document.querySelector('.server-error-test-form input');
		ajaxURL = o.metaDataRootDir + 'includes/forms/server-error-test/server_error_test_process.php';

		contactFormMutate();

	};

	(function (form) {
		if (form) {
			if (!form.classList.contains('server-error-test-form-on-submit-listener')) {
				form.classList.add('server-error-test-form-on-submit-listener');
				form.addEventListener('submit', formOnSubmit);
			}
		}
	}(document.querySelector('.server-error-test-form')));

};

returnMetaData = function (o) {

	var _canonical;
	var _default;
	var _description;
	var _image;
	var _imageAlt;
	var _page;
	var _title;
	var metaData;

// NB: Ensure that _description below matches $description in the corresponding meta-schema-overrides.inc.php for this page.

	_canonical = '${CANONICAL}';
	_default = '${DEFAULT}';
	_title = '${TITLE}';
	_description = 'Server Error Test page description for ' + o.siteData.metaDescription;
	_page = 'server-error-test';
	_image = _default;
	_imageAlt = _default;

	metaData = {
		page: _page,
		name: {
			"description": _description,
			"twitter:card": _default,
			"twitter:creator": _default,
			"twitter:description": _description,
			"twitter:site": _default,
			"twitter:title": _title,
			"twitter:url": _canonical,
			"twitter:image": _image,
			"twitter:image:alt": _imageAlt
		},
		property: {
			"og:description": _description,
			"og:image": _image,
			"og:image:alt": _imageAlt,
			"og:image:height": _default,
			"og:image:width": _default,
			"og:image:secure_url": _image,
			"og:image:type": _default,
			"og:site_name": _default,
			"og:title": _title,
			"og:type": _default,
			"og:url": _canonical
		}
	};


	return metaData;

};


// qwer: for reCAPTCHA: restore recaptchaEdits to module export:
export default Object.freeze({
	formLogic,
	main,
	returnMetaData
});
