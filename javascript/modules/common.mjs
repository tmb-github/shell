...
/*
		var ajaxEvent = new CustomEvent('ajaxResponse', { detail: {data: data, ajaxURL: ajaxURL, eventType: eventType, hrefText: hrefText, backbutton: backbutton, target: target, URL: URL, o: o} });
...
//console.log(o.siteData.localhostUrl);
...
// 2022-03-09
// Only do mouseover ajax on live site:
				//mouseoverPreload = (window.location.origin === o.siteData.liveSiteUrl);
...
// Cookie “_gat_gtag_UA_141513157_1” will be soon rejected...
...
//console.log('adding http404 listener');
...
// 2021-12-13:
// Only perform this on localhost:
//	if ((o.siteData && o.siteData.localhostUrl) && (window.location.origin === o.siteData.localhostUrl)) {
...
// PAGES: GALLERY
// needed regardless of landing page:
// This is called in siteWideEditsClosure():
...
// Determine if we're using hashed or non-hashed painting names/URIs:
// o.metaNameWebAuthor is defined in the outer function:
	if (o.metaNameWebAuthor && o.metaNameWebAuthor.dataset) {
		if (o.metaNameWebAuthor.dataset.hashedArtLinks) {
			o.hashedArtLinks = (o.metaNameWebAuthor.dataset.hashedArtLinks === 'true');
		} else {
			o.hashedArtLinks = false;
		}
	}

// Retrieve all gallery names from the navigation in the header. This will be
// present on all pages, so it's not specific to an initial gallery page view:
	document.querySelectorAll('.galleries .internal-anchor').forEach(function (element) {
		var hrefParts = element.href.split('/');
		var gallery = hrefParts[hrefParts.length - 2];
		o.galleries.push(gallery);
	});

// Collect all hidden gallery names from web_author meta tag and push to array:
	if (o.metaNameWebAuthor && o.metaNameWebAuthor.dataset) {
		if (o.metaNameWebAuthor.dataset.hiddenGalleries) {
			hiddenGalleries = o.metaNameWebAuthor.dataset.hiddenGalleries.split(' ');
			hiddenGalleries.forEach(function (gallery) {
				o.galleries.push(gallery);
			});
		}
	}

// Push the default hidden gallery onto the array:
	o.galleries.push('artwork-preview');
	o.galleries.push('all-works');
...
//
// According to the above, all we have to do here to show the user navigating about the site using the SPA mechanism is:
//
// o.gtag('set', 'page', window.location.pathname);
// o.gtag('send', 'pageview');
//
// But it doesn't work (2021-03-22).
//
// It seems we must reinitialize:
...
//	o.fenestra.location.gallery = '';
//	o.fenestra.location.painting = '';
//	o.fenestra.location.galleryPainting = '';

//		pathParts = o.fenestra.location.pathname.split('/');
//		gallery = pathParts[pathParts.length - 2];
//		painting = pathParts.pop();
