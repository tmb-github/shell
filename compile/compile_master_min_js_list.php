...
$loader_template_js = $absolute_root . 'javascript/scripts/loader.js';
...
// This is necessary so that we don't accidentally capture a JS file in a comment
	...
	for ($i = 0; $i < count($matches[0]); $i++) {
		...
		if (substr($resource, 0, 10) === 'javascript') {
			...
			$resource = str_replace('javascript/scripts', 'javascript/minified-scripts', $resource);

			$resource = str_replace('.js', '.min.js', $resource);
			$resource = str_replace('.min.min', '.min', $resource);
/*
			if ($resource === 'javascript/scripts/siteWideEditsRedux.js') {
				$resource = 'javascript/minified-scripts/siteWideEditsRedux.min.js';
			}
*/
	...
	file_put_contents($absolute_root . 'javascript/minified-scripts/master.min.js', $master);
